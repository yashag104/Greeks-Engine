...
module fp_exp #(
    ...
    wire signed [WL-1:0] INV_LN2 = $signed(  (1.4426950408889634 * (2.0**FL)) );

    // Polynomial coefficients for exp(r) ≈ c0 + c1*r + c2*r² + c3*r³ + c4*r⁴ + c5*r⁵
    // Minimax approximation on [-ln(2)/2, ln(2)/2]
    wire signed [WL-1:0] C0 = $signed( (1.0                  * (2.0**FL)) ); // 1.0
    wire signed [WL-1:0] C1 = $signed( (1.0                  * (2.0**FL)) ); // 1.0
    wire signed [WL-1:0] C2 = $signed( (0.5                  * (2.0**FL)) ); // 1/2!
    wire signed [WL-1:0] C3 = $signed( (0.16666666666666666   * (2.0**FL)) ); // 1/3!
    wire signed [WL-1:0] C4 = $signed( (0.041666666666666664  * (2.0**FL)) ); // 1/4!
    wire signed [WL-1:0] C5 = $signed( (0.008333333333333333  * (2.0**FL)) ); // 1/5!
        ...
        if (rst) begin
            ...
            case (state)
                ...
                S_RANGE_RED: begin
                    ...
                    // Extract integer part of x/ln(2)
                    // After multiply, result is in Q(2*IL, 2*FL)
                    // We want the integer part, which is bits [2*FL + IL - 1 : 2*FL]
                    k_val <= wide_product[2*FL + IL - 1 : 2*FL];
                    
                    // r = x - k * ln(2)
                    // First compute k * ln(2)
                    r_val <= x - k_val * LN2[FL-1:0]; // Approximate
